...
const CameraController = ({ target, onArrived }: CameraControllerProps) => {
  ...
  useFrame(() => {
    if (target) {
      ...
      const goalPos = target.clone().add(new THREE.Vector3(2, 1.5, 4));
      camera.position.lerp(goalPos, LERP_SPEED);
      lookTarget.current.lerp(target, LERP_SPEED);
